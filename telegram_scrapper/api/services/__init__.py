...
class ReportsService:
    ...
    def trends(self, past_days, limit, force_generation=False):
        report_id = f"general_trends_{past_days}"
        try:
            report = Report.objects.get(id=report_id)
            if force_generation or (
                datetime.now(timezone.utc) - report.updated_at
                > timedelta(hours=ReportsService.MESSAGES_PER_DAY_CACHE_IN_HOURS)
            ):
                data = self._generate_trends_report(report_id, past_days, limit)
            else:
    ...
    def top_users(self, past_days, limit, force_generation=False):
        ...
        try:
            ...
            if force_generation or (
                datetime.now(timezone.utc) - report.updated_at
                > timedelta(hours=ReportsService.TOP_USERS_CACHE_IN_HOURS)
            ):
    ...
    def group_activity(self, group_id, past_days=90, force_generation=False):
        ...
        try:
            ...
            if force_generation or (
                ...
                data = self._generate_group_activity_report(
                    report_id, group_id, past_days
                )
            else:
                data = json.loads(report.report_data)
        except Report.DoesNotExist:
    ...
    def _generate_group_activity_report(self, report_id, group_id, past_days):
        start_date = datetime.today() - timedelta(days=past_days)
        activity = list(
            Message.objects.filter(group=group_id, sent_at__gte=start_date.date())
            .order_by("-sent_at")
            .annotate(date=TruncDate("sent_at"))
            .order_by("date")
            .values("date")
    ...
    def _generate_top_users_report(self, report_id, past_days, limit):
        ...
        Report.objects.update_or_create(
            id=report_id,
            defaults={
                "report_data": json.dumps(result_data, cls=DjangoJSONEncoder),
                "updated_at": timezone.now(),
            },
        )
    ...
    def _generate_trends_report(self, report_id, past_days, limit):
        result_data = TrendsService().word_frequency(limit, past_days)
        Report.objects.update_or_create(
            id=report_id,
            defaults={
                "report_data": json.dumps(result_data, cls=DjangoJSONEncoder),
